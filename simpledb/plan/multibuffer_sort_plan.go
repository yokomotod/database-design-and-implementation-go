...
func (sp *MultibufferSortPlan) Open() (query.Scan, error) {
	...
	sp.logger.Tracef("BufferNeedsBestRoot(available=%d, size=%d) = %d", available, size, numBuffs)

	src, err := sp.plan.Open()
	if err != nil {
		return nil, fmt.Errorf("sp.plan.Open: %w", err)
	}
	runs, err := sp.multibufferSplitIntoRuns(src, numBuffs)
	if err != nil {
		return nil, fmt.Errorf("sp.splitIntoRuns: %w", err)
	}
	src.Close()
	for int32(len(runs)) > numBuffs {
	...
	sp.logger.Tracef("NewMultibufferSortPlan(): len(runs)=%d", len(runs))
	sc, err := query.NewMultibufferSortScan(runs, sp.comp)
	if err != nil {
		return nil, fmt.Errorf("query.NewMultibufferSortScan: %w", err)
	}
	return sc, nil
}
