...
func (hjp *HashJoinPlan) Open() (query.Scan, error) {
	...
	// plans := make([]Plan, len(buckets1))
	// for i := range buckets1 {
	// 	var p Plan
	// 	p = NewMultibufferProductPlan(hjp.tx, buckets1[i], buckets2[i])
	// 	p, err = NewSelectPlan(
	// 		p,
	// 		query.NewPredicateWithTerm(query.NewTerm(
	...
	return query.NewHashJoinScan(hjp.tx, buckets1, buckets2), nil
}
...
func (hjp *HashJoinPlan) splitIntoBucket(p *query.TempTable, numBuffs, mod int32, fldName string) ([]*query.TempTable, error) {
	...
	for i := range buckets {
		temp := query.NewTempTable(hjp.tx, p.Schema())
		scan, err := temp.Open()
		if err != nil {
	...
	for {
		...
		for _, fldname := range p.Schema().Fields() {
			val, err := scan.GetVal(fldname)
			if err != nil {
				return nil, fmt.Errorf("scan1.GetVal: %w", err)
			}
